...
{        // GrabCut segmentation
		Mat result;
        Mat bgModel,fgModel; 
		Rect rectangle(5,10,image.cols-10,image.rows-10);
...
Mat remove_line(Mat image){
	int loop = 0,key =0;
	Mat im2;
	image.copyTo(im2);
	float area = im2.rows*im2.cols;
	Mat mask = Mat::zeros(image.rows+2,image.cols+2,CV_8UC1);
	Point arr[3] = {(0,0),((image.rows/2),(image.cols/2)),((image.rows-2),(image.cols-2))};
	
xy:  cout<<" inside\n";
	floodFill(image, mask, arr[key], 255); 
	image.copyTo(im2);
	erode(im2, im2, Mat(3,3,CV_8UC1));
	bitwise_not(im2, im2);

	//key=0;

	vector< vector<Point>> flood_contours;
	findContours(im2, flood_contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
	for(int i=0; i<flood_contours.size();i++){
		if(contourArea(flood_contours[i])>0.65 *area)  key++; 
		loop++;
	}
	if(loop>=3) key=0;
	if(key>0) goto xy;
	
	namedWindow("floodfill",CV_WINDOW_NORMAL);imshow("floodfill",im2);
	return im2;
}
		...
		if(contourArea(contours[i])>250 && contourArea(contours[i])<20000){
			//if(isContourConvex(contours[i]))
					...
					if(is_inside){
						...
						Mat img = GrabCut(crop);
						roi.push(crop);
						size = to_string(roi.size());
						displayMatImage("roi "+size,img); imwrite("roi "+size+".jpg",img);
					}else{
						...
						Mat mat_zero = (img != Mat::zeros(img.size(),CV_8UC3));
						roi.push(crop);
						size = to_string(roi.size());
						displayMatImage("roi "+size,img); imwrite("roi "+size+".jpg",img);
					}
